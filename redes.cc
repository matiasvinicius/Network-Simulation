...
int main(int argc, char *argv[]){
  ...
  CommandLine cmd;
  cmd.AddValue("numCsma", "Numeros de nos extras rede broadcast", numCsma); //permite mudar o numero de nós da rede csma (broadcast)
  cmd.Parse(argc, argv);
  ...
  //Interfaces de Rede
  NetDeviceContainer p2pDevice01;
  p2pDevice01 = pointToPoint.Install(p2pNode01);
  NetDeviceContainer p2pDevice02;
  p2pDevice02 = pointToPoint.Install(p2pNode02);
  NetDeviceContainer p2pDevice03;
  p2pDevice03 = pointToPoint.Install(p2pNode03);
  NetDeviceContainer p2pDevice12;
  p2pDevice12 = pointToPoint.Install(p2pNode12);
  ...
  double posY = 10.0;
  for(int i = 0; i < numP2P -1; i++){
    anim.SetConstantPosition (p2pNodeGeral.Get(i), posX, posY);
    posX += 20.0;
  }

  //define posições do(s) node(s) csma (broadcast) no NetAnim
  posX = 10.0;
  posY = 20.0;
