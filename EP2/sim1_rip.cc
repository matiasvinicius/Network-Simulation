...
int main(int argc, char *argv[]){
  ...
  */
  ...
  /*Muda-se o custo de um enlace antes da instalação da pilha de internet nos roteadores */
  /*As linhas comentadas abaixo liberam uma das simulações do EP*/
  /*SIMULAÇÃO DE CUSTO 8 NA INTERFACE 1 (192.169.0.0) DO ROTEADOR 4
  ripRouting.SetInterfaceMetric (routers.Get(4), 1, 8);
  ripRouting.SetInterfaceMetric (routers.Get(5), 1, 8);
  */
  /*SIMULAÇÃO DE CUSTO 3 NAS INTERFACES 2 (192.168.3.0) E 3 (192.168.5.0) DO ROTEADOR 0
  ripRouting.SetInterfaceMetric (routers.Get(0), 3, 3);
  ripRouting.SetInterfaceMetric (routers.Get(0), 2, 3);
  ripRouting.SetInterfaceMetric (routers.Get(2), 1, 3);
  ripRouting.SetInterfaceMetric (routers.Get(3), 1, 3);
  */
  /*SIMULAÇÃO DE CUSTO 13 NA INTERFACE 1 (192.169.0.0) DO ROTEADOR 4*/
  ripRouting.SetInterfaceMetric (routers.Get(4), 1, 13);
  ripRouting.SetInterfaceMetric (routers.Get(5), 1, 13);
  ...
  routingHelper.PrintRoutingTableAt (Seconds (40), routers.Get(4), routingStream);
  
  //Rota realizada no arquivo .tr
  AsciiTraceHelper ascii;
  channelLevel2.EnableAsciiAll (ascii.CreateFileStream ("sim1_rip.tr"));
