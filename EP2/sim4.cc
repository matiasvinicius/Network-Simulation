...
int main(int argc, char *argv[]){
  ...
  */
  ...
  //Define o protocolo AODV como prioridade na lista de protocolos de roteamento para os roteadores 6 até 8
  //Define o protocolo RIP como prioridade na lista de protocolos de roteamento para os roteadores 0 até 5
  ...
  //Instala o protocolo de roteamento AODV nos roteadores 6 -> 8
  for(int i = 6; i <= 8; i++){
    internetRouters.Install(routers.Get(i));
  }

  RipHelper rip;
  Ipv4ListRoutingHelper rout2;
  rout2.Add(rip,0);
  ...
  //Instala o protocolo de roteamento RIP nos roteadores 0 -> 5
  for(int i = 0; i <= 5; i++){
    internetRouters2.Install(routers.Get(i));
  }
  ...
  Ipv4InterfaceContainer destinatario = address.Assign (r5r6_link);   
  ...
  //print dos roteadores AODV
  AodvHelper aodvHelper;
  Ptr<OutputStreamWrapper> routingStream = Create<OutputStreamWrapper> (&std::cout);
  for(int time=0.0; time <= 100; time+=10){
    //aodvHelper.PrintRoutingTableAt (Seconds (time), routers.Get(0), routingStream);
    aodvHelper.PrintRoutingTableAt (Seconds (time), routers.Get(6), routingStream);
  }

  //print dos roteadores RIP
  ...
  channelLevel2.EnablePcapAll("sim4");


  //--------Animação do NetAnim--------

  //Gera xml para usar no NetAnim
  AnimationInterface anim ("sim4.xml");
